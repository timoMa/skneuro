...
class RatioPolicy{
    ...
    public:
        ...
        typedef typename vigra::NumericTraits<PIXEL_TYPE_IN>::RealPromote PixelType;
        typedef typename vigra::NumericTraits<PIXEL_TYPE_IN>::ValueType   ValueType;
        ...
        bool usePixel(const PixelType & meanA, const PixelType & varA)const{
            return vigra::sum(meanA) > epsilon_  &&  vigra::sum(varA) > epsilon_;
        }
        ...
        bool usePixelPair(
            ...
            const ValueType m = vigra::mean(meanA/meanB);
            const ValueType v = vigra::mean(varA/varB);          
...
inline bool equal(const vigra::TinyVector<V,SIZE> & a,const vigra::TinyVector<V,SIZE> b){
...
class NormPolicy{
    ...
    public:
        ...
        bool usePixel(const PixelType & meanA, const PixelType & varA)const{
            return vigra::sum(varA)>epsilon_;
        }
        ...
        bool usePixelPair(
            ...
            // Compute mean ratio of mean and variance
            const ValueType m = vigra::squaredNorm(meanA-meanB);
            const ValueType v = vigra::mean(varA/varB); 
        ...
        ValueType distanceToWeight(const PixelType & meanA, const PixelType & varA, const ValueType distance){
            return  vigra::exp(-distance /sigmaSquared_);
        }
...
public:
    ...
    typedef vigra::MultiArrayView<DIM,RealPromotePixelType>  MeanArrayView;
    typedef vigra::MultiArrayView<DIM,RealPromotePixelType>  VarArrayView;
    typedef vigra::MultiArrayView<DIM,RealPromotePixelType>  EstimateArrayView;
    typedef vigra::MultiArrayView<DIM,RealPromoteScalarType> LabelArrayView;
    ...
    // range type
    typedef vigra::TinyVector<int,2> RangeType;
    //typedef boost::mutex      MutexType;

    typedef vigra::threading::thread  ThreadType;
    typedef vigra::threading::mutex   MutexType;
    ...
    BlockWiseNonLocalMeanThreadObject(
        ...
        vigra::MultiArray<1,int> &  progress
    )
    : 
    inImage_(inImage),
...
private:
    ...
    vigra::MultiArrayView<1,int>  progress_;
...
inline void BlockWiseNonLocalMeanThreadObject<DIM, PIXEL_TYPE_IN, SMOOTH_POLICY>::initalizeGauss(){
    ...
    vigra::Gaussian<RealPromoteScalarType> gaussian(param_.sigmaSpatial_);
    int c=0;
...
inline void BlockWiseNonLocalMeanThreadObject<DIM, PIXEL_TYPE_IN, SMOOTH_POLICY>::processSinglePixel(
    const Coordinate & xyz
        ...
        Coordinate nxyz(vigra::SkipInitialization);
...
BlockWiseNonLocalMeanThreadObject<DIM,PIXEL_TYPE_IN,SMOOTH_POLICY>::patchDistance(
    ...
    Coordinate offset(vigra::SkipInitialization),nPa(vigra::SkipInitialization),nPb(vigra::SkipInitialization);
    int acu = 0;
...
BlockWiseNonLocalMeanThreadObject<DIM,PIXEL_TYPE_IN,SMOOTH_POLICY>::patchExtractAndAcc(
    ...
    Coordinate xyzPos(vigra::SkipInitialization),abc(vigra::SkipInitialization);
...
BlockWiseNonLocalMeanThreadObject<DIM,PIXEL_TYPE_IN,SMOOTH_POLICY>::patchAccMeanToEstimate(
    ...
    Coordinate abc(vigra::SkipInitialization),xyzPos(vigra::SkipInitialization),nhSize(param_.patchRadius_);        
    int count = 0 ;
    const int ns = 2 * param_.patchRadius_ + 1;
...
void nonLocalMean1Run(
    ...
    {   // MULTI THREAD CODE STARTS HERE
        ...
        vigra::MultiArray<1,int> progress = vigra::MultiArray<1,int>(typename  vigra::MultiArray<1,int>::difference_type(nThreads));
