...
#undef MAX____
#endif
#define MAX____(a, b) ((a)>(b)?(a):(b))
#define absd____(a) ((a)>(-a)?(a):(-a))
#ifndef pow2___
#define pow2___(a) (a*a)
#endif
#define n___ 3
...
/*  Fortran subroutine in EISPACK. */
    ...
    double f, g, h;
    double hh;
    for (j = 0; j < n___; j++) {d[j] = V[n___-1][j]; }
    ...
    for (j = 0; j < n___; j++) { d[j] = V[n___-1][j]; V[n___-1][j] = 0.0; }
    V[n___-1][n___-1] = 1.0;
    e[0] = 0.0;
}

/* Symmetric tridiagonal QL algorithm. */
static void tql2(double V[n___][n___], double d[n___], double e[n___]) {
...
/*  Fortran subroutine in EISPACK. */
    ...
    for (l = 0; l < n___; l++) {
        ...
        tst1 = MAX____(tst1, fabs(d[l]) + fabs(e[l]));
        m = l;
        while (m < n___) {
...
inline int fast_eigen3x3(double A[3][3], double V[3][3], double d[3])
{
    ...
    double da[3];
    check=(absd____(A[0][1])<smallv)+(absd____(A[0][2])<smallv)+(absd____(A[1][2])<smallv);
    if(check>1) { return 0; }

    /* 0 = - det (A - yI) = y^3 + c2 * y^2 + c1 *y + c0 */
    c0 = -(A[0][0]*A[1][1]*A[2][2] + 2*A[0][1]*A[0][2]*A[1][2] - A[0][0] * pow2___(A[1][2]) - A[1][1]*pow2___(A[0][2]) - A[2][2]*pow2___(A[0][1]));
    c1 = A[0][0]*A[1][1] - pow2___(A[0][1]) + A[0][0]*A[2][2] -pow2___(A[0][2]) + A[1][1]*A[2][2] - pow2___(A[1][2]);
    c2 = - (A[0][0] + A[1][1] + A[2][2]);

    /* Solve the roots of  y^3 + c2 * y^2 + c1 *y + c0  */
    roots3(d, c0, c1, c2);

    da[0]=absd____(d[0]); da[1]=absd____(d[1]); da[2]=absd____(d[2]);
    /* Sort eigenvalues */
    if(da[0]>=da[1])
    {
        if(da[0]>da[2])
    ...
    a1=A[0][1]*A[1][2]; a2=A[0][1]*A[0][2]; a3=pow2___(A[0][1]);
    ...
    l1=sqrt(pow2___(V[0][0])+ pow2___(V[1][0]) + pow2___(V[2][0]));
    l2=sqrt(pow2___(V[0][1])+ pow2___(V[1][1]) + pow2___(V[2][1]));
    l3=sqrt(pow2___(V[0][2])+ pow2___(V[1][2]) + pow2___(V[2][2]));
